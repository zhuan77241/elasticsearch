...
JSON 结构对于电脑非常棒。即使它的 pretty-printed，试图找到数据中的关系也很冗长乏味。
人眼，特别是在看 ssh 终端时，需要紧凑和对齐的文本。
cat API 旨在满足这一需求。

所有的 cat 命令都接受一个字符串参数 `help` 来查看全部
它们提供的头文件和信息， `/_cat` 命令单独列出了所有可用的命令。
...
您也可以使用简单的通配符来请求多个列 `/_cat/thread_pool?h=ip,bulk.*`
获取所有头部（或别名）以 `bulk.` 开头。

[float]
[[numeric-formats]]
=== 数字格式

许多命令提供几种类型的数字输出，不管是字节，大小或时间。
默认情况下，这些类型是 human-formatted，例如， `3.5mb` 而不是 `3763212` 。
这些人类可读的值无法以数值排序，所以为了操作这些在顺序上很重要的数值，你可以改变它。.

假设您想要查找群集中的最大索引（所有分片使用的存储空间，而不是文档数量）。
...
或者，您可以设置 "Accept" HTTP 头为适当的媒体格式。
